	...
	if len(h.Firewall) > 0 {
		h.addSystemPackage("firewall",
			h.newHostPackage(
				...
				cmd.WriteAsset("/etc/network/if-pre-up.d/iptables", "fw_upstart.sh", "root", 0744),
				cmd.WriteAsset("/etc/iptables/ipsets", "fw_ipset.conf", "root", 0644),
				cmd.WriteAsset("/etc/iptables/rules_ipv4", "fw_rules_ipv4.conf", "root", 0644),
				cmd.WriteAsset("/etc/iptables/rules_ipv6", "fw_rules_ipv6.conf", "root", 0644),
